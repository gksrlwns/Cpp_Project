...
void Day24Creature::Update_Attack()
{
	...
	// 내가 원하는 Flipbook 인덱스가 지나갔으면
	// (나중에 엔진을 실제로 사용할 때는)
	// 엔진 기능으로도 부족해서
	// 툴을 새로 만듬
	// 이거는 프로그래머 보다 기획/아트 에서 많이 사용하기 때문에
	// 원하는 스펙대로 툴을 만들어서 줍니다. (스킬 툴)
	// 
	// 스킬툴에 들어가는 내용들
	// - 어떤 프레임에
	// - 어떤 이벤트를 설정하고
	// - 그 이벤트로 인해 어떤 것들이 발동하는지 (소환, 힐, 광역데미지)
...
#pragma region 상태변환 AI
	//Attack이 마지막 프레임이 아니면 상태가 절대 안바뀜
	//Death 제외
...
void Day24Creature::SetInputDir(Vector2 dir)
{
	...
	//SetDir 호출을 위한 작업
	//8방향이면 상관없지만 4방향임
	// w,a키 같이 눌렀을 때 대각선 이동은 되지만 방향은 둘중 하나만 볼 수 있음(리소스 한계)
	//4방향 뿐임
